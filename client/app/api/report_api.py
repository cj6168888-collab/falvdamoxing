...
金额表不宜只给一个总额。应制作三版：保守版用于立案和胜诉确定性评估，完整主张版用于谈判和调解，备位版用于法院拆分法律关系审查。每一笔金额都应附证明对象、证据编号、计算公式、对方可能抗辩和补证动作。
...
## 八、结论
本案当前证据体量足以支持深度诉讼准备，但胜负关键在证据结构化和请求权拆分。系统不应把 177 条证据直接堆给用户，而应持续输出可执行的诉讼产品：证据目录、时间线、金额表、补证清单、庭审发问提纲和分层诉讼请求。该方向与用户对 AI 律师的期望一致：不是泛泛解释法律，而是把证据链转化为可审查、可提交、可谈判、可庭审使用的法律工作成果。"""
