...
对抗分析的第一目标，是把 177 条材料从“材料堆”转化为法院能够审查的请求权结构：每一项诉求必须对应独立事实、证据组合、法律依据、金额计算和对方可能攻击点。第二目标，是提前识别对方最可能采用的拆分抗辩，防止我方把合作出资、工资社保、保证金、信息服务费、物业水电、报销款等不同法律关系混合表达，导致裁判者按证明不足或请求权基础不明处理。
...
## 四、案件走向预测
有利路径是把 177 条材料转化为“主请求-备位请求-证据目录-金额表-时间线”五件套：每项诉求都对应证据名称、证明事实、法律依据和对方攻击点。法院或仲裁机构最容易支持的部分，通常是主体明确、金额可计算、履行或付款事实闭环完整的请求；最容易被压缩的部分，是法律性质模糊、受益主体不明、只有单方统计表而缺少合同或审批依据的请求。
...
7. 对金额表做三层版本：保守主张、完整主张、备位主张。保守版本用于提高胜诉确定性，完整版本用于谈判锚定，备位版本用于应对法院拆分审查。
...
同步入口已经给出可稳定返回的完整攻防框架；如需更长篇幅的多轮沙盘推演、逐项证据质证和诉讼策略迭代，应继续使用异步 full-analysis-async，并通过任务状态接口获取完整结果。对用户而言，本报告的直接使用方式是：先把“行动方案”转为任务清单，再把“攻防矩阵”转为证据目录和庭审提纲，最后用“案件走向预测”校验每项请求的胜诉概率和谈判价值。"""
...
【综合执行建议】
1. 优先围绕对手律师已提出的攻击点补强证据目录，逐项对应合同成立、履行、付款、损失与因果关系。
        ...
        for round_no in range(4, debate_round_limit):
            if round_no % 2 == 0:
                ...
                followup_result = llm_service.opponent_analysis(
                    case_info=辩论状态存储[debate_id]["case_info"],
                    opponent_info=f"对方已看到我方如下回应，请继续寻找可攻击点：\n{current_exchange}"
                )
                speaker = "🔴 对手律师"
