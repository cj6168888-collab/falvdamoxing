# -*- coding: utf-8 -*-
"""
法律大模型辅助系统 - 端到端测试脚本 v2
E2E Test for Legal LLM System - Corrected API paths
"""
import sys
import os
import json
...
LOG_FILE = r'D:\www\法律大模型\e2e_test_report_v2.json'
...
try:
    case_data = {
        "title": f"E2E_Test_v2_{datetime.now().strftime('%H%M%S')}",
        "case_type": "civil",
        "plaintiff": "测试原告",
        "defendant": "测试被告",
        "cause": "合同纠纷",
...
if case_id:
    try:
        ...
        if r.status_code == 200:
            result = r.json()
            add_result("Case Profile", True, result)
        else:
...
if case_id:
    try:
        r = requests.get(f"{BASE_URL}/api/时间把控/case/{case_id}/deadlines", timeout=10)
        if r.status_code == 200:
            result = r.json()
            add_result("Time Control API", True, {"deadlines": len(result) if isinstance(result, list) else 0})
        else:
            add_result("Time Control API", False, error=f"Status: {r.status_code}")
    except Exception as e:
        add_result("Time Control API", False, error=e)
else:
    add_result("Time Control API", False, error="No case_id available")

# 测试11: 对抗性分析 - 使用异步端点避免超时
log("测试11: 对抗性分析")
if case_id:
    try:
        # 使用异步端点避免超时
        ...
        r = requests.post(f"{BASE_URL}/api/对抗性分析/case/{case_id}/full-analysis-async", json=analysis_data, timeout=30)
        if r.status_code == 200:
            result = r.json()
            add_result("Adversarial Analysis", True, {"has_result": bool(result), "task_id": result.get("task_id")})
        else:
            add_result("Adversarial Analysis", False, error=f"Status: {r.status_code}, Response: {r.text[:200]}")
    except Exception as e:
        add_result("Adversarial Analysis", False, error=e)
else:
    add_result("Adversarial Analysis", False, error="No case_id available")

# 测试12: 报告生成 API - 增加超时时间
log("测试12: 报告生成 API")
if case_id:
    try:
        r = requests.post(f"{BASE_URL}/api/reports/generate/{case_id}", json={}, timeout=120)
        if r.status_code in [200, 201]:
            result = r.json()
            add_result("Report Generation", True, {"has_result": bool(result)})
        else:
            add_result("Report Generation", False, error=f"Status: {r.status_code}")
    except Exception as e:
...
if case_id:
    try:
        ...
        r = requests.post(f"{BASE_URL}/api/v2/senior-analysis/analyze", json=analysis_data, timeout=30)
        if r.status_code == 200:
            result = r.json()
...
if case_id:
    try:
        r = requests.post(f"{BASE_URL}/api/时间把控/case/{case_id}/generate-milestones", json={}, timeout=120)
        if r.status_code == 200:
            result = r.json()
            add_result("Milestone Generation", True, {"has_result": bool(result)})
        else:
            add_result("Milestone Generation", False, error=f"Status: {r.status_code}")
    except Exception as e:
        add_result("Milestone Generation", False, error=e)
else:
    add_result("Milestone Generation", False, error="No case_id available")

# 测试18: 证据图谱 - 使用正确的API路径
log("测试18: 证据图谱 API")
if case_id:
    try:
        # 证据图谱V2的路径是 /api/v2/evidence-graph/graph/data
        r = requests.get(f"{BASE_URL}/api/v2/evidence-graph/graph/data?case_id={case_id}", timeout=30)
        if r.status_code == 200:
            result = r.json()
            add_result("Evidence Graph", True, {"has_result": bool(result)})
        else:
            add_result("Evidence Graph", False, error=f"Status: {r.status_code}, Path: /api/v2/evidence-graph/graph/data")
    except Exception as e:
        add_result("Evidence Graph", False, error=e)
else:
    add_result("Evidence Graph", False, error="No case_id available")
...
# 测试20: API 文档可用性
log("测试20: API 文档（Swagger UI）")
try:
    r = requests.get(f"{BASE_URL}/docs", timeout=10)
    if r.status_code == 200:
        add_result("API Documentation", True, {"content_type": r.headers.get("content-type", "")})
    else:
        add_result("API Documentation", False, error=f"Status: {r.status_code}")
except Exception as e:
    add_result("API Documentation", False, error=e)
...
log(f"总计测试: {total}")
log(f"通过: {passed} ({passed*100//total if total > 0 else 0}%)")
log(f"失败: {failed} ({failed*100//total if total > 0 else 0}%)")
log("")
